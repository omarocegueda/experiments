...
segbase="${reference%.*}"
segbase="${segbase%.*}"
# Execute K-means segmentation, request to save the file as 'seg_X.nii.gz' where X is the name of the input image (after removing the .nii.gz extension)
python kmeans.py img/$img seg_$imgbase.nii.gz
